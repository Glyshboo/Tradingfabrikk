...
class MasterEngine:
    ...
    async def _decision_loop(self) -> None:
        while True:
            ...
            if self.cfg.get("mode") == "live" and not self.data.is_account_sync_healthy(
                max_age_sec=float(self.cfg["engine"].get("account_stale_after_sec", self.cfg["engine"]["stale_after_sec"] * 3)),
                require_event=True,
            ):
                self.risk.trigger_safe_pause()
                log_event("safe_pause", {"reason": "account_sync_stale"})
                self._write_status("SAFE_PAUSE")
                await asyncio.sleep(1)
                continue
    ...
    async def _execute_decision(self, decision: DecisionRecord) -> None:
        ...
        confidence = float(decision.sizing.get("confidence", 0.0) or 0.0)
        if confidence <= 0.0:
            decision.blocked_reason = "invalid_sizing_confidence"
            decision.caps_status = {
                "safe_pause": self.risk.safe_pause,
                "reduce_only_mode": self.risk.reduce_only_mode,
                "risk_result": "blocked_pre_risk",
            }
            self.audit.save_decision(decision)
            log_event("decision_blocked", decision.as_audit_payload())
            return
        qty = max(0.0, self.cfg["sizing"]["base_qty"] * confidence)
        decision.side = side
        decision.qty = qty
        ...
        self.position_mgr.on_entry(
            ...
            fill_price,
            {
                ...
                "time_stop_bars": int(decision.sizing.get("time_stop_bars", 0) or 0),
            },
        )

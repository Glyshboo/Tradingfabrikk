...
class MasterEngine:
    ...
    async def _decision_loop(self) -> None:
        while True:
            ...
            for symbol in self.cfg["symbols"]:
                ...
                exposure_penalty = min(0.05, max(0.0, self.risk._exposure(self.account, self.data.market) / max(self.cfg["risk"]["max_total_exposure_notional"], 1)))
                decision = self.selector.select(symbol, regime, candidates, cost_proxy, exposure_penalty=exposure_penalty)
                if not decision:
                    continue
    ...
    async def _execute_decision(self, decision: DecisionRecord) -> None:
        ...
        decision.sizing["qty"] = round(qty, 8)
        decision.sizing["max_total_exposure_notional"] = self.cfg["risk"]["max_total_exposure_notional"]
        ...
        if not rr.allowed:
            ...
            log_event("decision_blocked", {"symbol": decision.symbol, "reason": rr.reason, "selected_side": decision.selected_side})
        ...
        self._update_position_after_fill(decision.symbol, side, qty)
        self.audit.save_decision(decision)
        log_event(
            "order_submitted",
            {
    ...
    def _update_position_after_fill(self, symbol: str, side: str, qty: float) -> None:
        pos = self.account.positions.setdefault(symbol, PositionState(symbol=symbol))
        signed = qty if side.upper() == "BUY" else -qty
        pos.qty += signed
        snap = self.data.market.get(symbol)
        if snap:
            pos.entry_price = snap.price

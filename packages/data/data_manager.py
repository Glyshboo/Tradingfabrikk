...
class DataManager:
    ...
    def is_account_sync_healthy(self, max_age_sec: float, require_event: bool) -> bool:
        if not self.user_stream_alive:
            return False
        last_event = self.account_state.get("last_event_ts")
        if last_event is None:
            return not require_event
        return (time.time() - float(last_event)) <= max_age_sec

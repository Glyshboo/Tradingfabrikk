...
class ResearchOptimizer:
    ...
    def walk_forward(self, prices: List[float], split: float = 0.7) -> Dict:
        bt = CandleBacktester()
        cut = max(2, int(len(prices) * split))
        ins = bt.run(prices[:cut], fee_bps=4, slippage_bps=2)
        oos = bt.run(prices[cut:], fee_bps=4, slippage_bps=2)
        report = {
            "split": split,
            "in_sample": {"trades": ins.trades, "pnl": ins.pnl, "sharpe_like": ins.sharpe_like},
            "out_of_sample": {"trades": oos.trades, "pnl": oos.pnl, "sharpe_like": oos.sharpe_like},
        }
        (self.out_dir / "walk_forward.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
        return report

...
def test_random_search_creates_candidates_per_symbol(tmp_path, monkeypatch):
    monkeypatch.setattr(DataManager, "_download_klines", lambda self, **kwargs: _fake_klines(kwargs.get("limit", 60)))
...
def test_random_search_score_changes_with_cost_model(tmp_path, monkeypatch):
    monkeypatch.setattr(DataManager, "_download_klines", lambda self, **kwargs: _fake_klines(kwargs.get("limit", 80)))
    search_space = {
        "atr_stop_mult": [2.0],
        "time_stop_bars": [12],
        "base_confidence": [0.58],

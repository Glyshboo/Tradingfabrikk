...
def test_selector_applies_funding_behavior_penalty():
    selector = StrategySelector(base_edge={"TrendCore": 0.1})
    signal = StrategySignal("BTCUSDT", "BUY", 0.8, 95.0, 110.0, "x")
    ...
    neutral = selector.select(
        ...
        exposure_penalty=0.0,
        current_positions={},
        symbol_profile=SymbolProfile(liquidity_signature=1.0, funding_behavior=0.0),
    )
    expensive = selector.select(
        "BTCUSDT",
        Regime.TREND_UP,
        [("TrendCore", "default", signal)],
        {"spread": 0.001, "slippage": 0.0, "funding": 0.0},
        exposure_penalty=0.0,
        current_positions={},
        symbol_profile=SymbolProfile(liquidity_signature=1.0, funding_behavior=0.02),
    )

    assert expensive.score_breakdown["TrendCore:default"] < neutral.score_breakdown["TrendCore:default"]
